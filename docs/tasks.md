...
| **数据模块** | JDBC 存取、实体、业务计算（BMI/体脂）、建库建表、配置加载 | `com.bmi.model.db`：UserDao / RecordDao / User / Record / JdbcUtil；`com.bmi.model.ai`：CalcUtil | FR-01, FR-03, FR-04, FR-05 |
...
| **开发B-数据** | 数据模块 | UserDao、RecordDao、User、Record、JdbcUtil、CalcUtil | SQLite 建表 DDL、DAO 实现、业务计算、db-config 模板 |
...
|------|----------|------------|----------|----------------|
| 07-14 | 工程骨架、`MainFrame` 窗口框架、`LoginView` 原型 | SQLite 建库建表 DDL、`JdbcUtil`、`User`/`Record` 实体、`UserDao` 骨架 | `ai-key.properties` 加载器、`AiRequest` 模型、`AiHealthClient` 接口骨架 | **M1**：库可连通 + 配置可加载 |
...
| 07-16 | `RecordInputPanel`（身高/体重/年龄/性别输入 + 区间校验） | `RecordDao` insert/queryByUser/deleteById；`CalcUtil` | `AiHealthClient` 响应体解析（choices[0].message.content） | **M3**：`RecordController` 串录入→BMI→体脂→落库（FR-02~04） |
...
| 07-22 | UI 回归与边界提示核对 | JUnit：`CalcUtil`/`UserDao`/`RecordDao` 单测 | JUnit：`AiHealthClient` 降级单测（Mock 超时/异常） | 单元测试完成 |
