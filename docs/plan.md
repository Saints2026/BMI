...
| 编写角色 | 架构师 Agent |
| 技术栈边界 | Java 8+ · JavaFX · JDBC + SQLite · HttpURLConnection · JUnit（jar 入 `lib/`，无 Maven/Gradle） |

## 2. 设计概述

系统为**桌面端自研 MVC** 应用：用户通过 **JavaFX** 界面录入身高/体重/年龄/性别，经控制器调度业务计算与数据访问层，完成 BMI 计算（FR-03）、体脂预测（FR-04）、记录落库（FR-05）、折线图渲染（FR-06，JavaFX `LineChart`）与 AI 健康建议（FR-07），并实现账号注册登录（FR-01）与录入校验（FR-02）。
...
```
   ...
   │ UserDao/RecordDao  │        │ AiService          │
   │ User/BodyRecord    │        │ BmiCalculator /    │
...
```
用户        InputView           RecordController   BmiCalculator   BodyFatCalculator   RecordDao      ChartController   ChartView
 │              │                    │                │                │                  │               │                │
 │─输入175cm/70kg/30岁/男─▶│            │                │                │                  │               │                │
...
| `MainView` | `void buildLayout(User user)` | 构建主窗口，组合各功能页 |
| `MainView` | `void switchToInputTab()` / `void switchToHistoryTab()` / `void switchToChartTab()` | 切换功能页签 |
| `InputView` | `void onSave(double heightCm, double weightKg, int age, int gender)` | 录入提交，触发控制器计算 |
| `InputView` | `void showValidationError(String field, String msg)` | 输入框旁校验错误（如「身高应在 50–250cm 之间」） |
| `InputView` | `void showResult(double bmi, String grade, double bodyFat)` | 展示 BMI/分级/体脂结果 |
| `MainView`（历史区） | `void renderTable(List<BodyRecord> records)` | 渲染历史记录表格 |
| `MainView`（历史区） | `void onDelete(long recordId)` | 请求删除指定记录 |
| `MainView`（历史区） | `void showEmptyHint()` | 无记录提示「暂无历史记录」 |
| `ChartView` | `void setMetric(int metric)` | 设置当前指标（0=BMI/1=体重/2=体脂率） |
| `ChartView` | `void repaint(List<BodyRecord> series)` | 重绘折线图（JavaFX LineChart） |
| `ChartView` | `void showChartHint(String msg)` | 数据不足提示（如「至少需 2 条记录才能显示趋势」） |
...
| `RecordController` | `BodyRecord createRecord(long userId, double h, double w, int age, int gender)` | 录入→计算 BMI/体脂→组装 BodyRecord→落库，返回完整记录 |
| `RecordController` | `List<BodyRecord> queryRecords(long userId, Date start, Date end)` | 按用户+时间区间查询（默认升序） |
| `RecordController` | `boolean deleteRecord(long userId, long recordId)` | 删除本人记录（越权拒绝） |
| `ChartController` | `List<BodyRecord> getSeries(long userId, int metric)` | 取历史记录集合供图表渲染 |
| `AiController` | `String getAdvice(long userId)` | 汇总最新指标+历史摘要→调用 AiService→返回建议文本 |
...
| `RecordDao` | `long insert(BodyRecord record)` | 插入测量记录，返回自增主键 |
| `RecordDao` | `List<BodyRecord> queryByUser(long userId, Date start, Date end)` | 按用户+时间区间升序查询 |
...
**实体 `BodyRecord`**（字段：`long id; long userId; Date measureTime; double height; double weight; double bmi; double bodyFat; int age; int gender; Date createdAt;`）
...
| `AiService` | `String requestAdvice(AiRequest req)` | 组装 JSON→HttpURLConnection 调用→解析→降级 |
| `AiService` | `AiRequest buildRequest(User u, BodyRecord latest, List<BodyRecord> history)` | 构造请求（system/userMetrics/historyTrend/modelParams） |
...
> 正式表名：`user`（用户）与 `body_record`（测量记录），详见 `docs/db_design.md` 完整双方言 SQL。
...
CREATE INDEX idx_record_user_time ON body_record(user_id, measure_time);
```

字段约束说明：`user.username` 唯一且非空；`body_record.user_id` 为外键关联 `user.id`，删除用户级联删记录；`measure_time/height/weight/bmi/body_fat` 均非空。
...
| 视图类 | 后缀 `XxxView`/`XxxPanel`/`XxxFrame` | `LoginView`、`InputView`、`ChartView`、`MainView` | `ChartUI` |
...
| 表名 | 小写下划线（无前缀） | `user`、`body_record` | `TUser`、`UserTable` |
...
| 维度 | 选型 | 理由 | 是否在白名单 |
|------|------|------|--------------|
| 图表 | **JavaFX LineChart**（`javafx.scene.chart.LineChart`） | JavaFX 原生图表组件、零额外依赖、满足 FR-06 切换/重绘 <500ms | ✅（宪章允许） |
...
| 功能 | 对应类/接口 | 对应表/接口 |
|------|-------------|-------------|
| FR-01 登录注册 | `LoginView`、`UserController`、`UserDao`、`User` | `user` |
| FR-02 身高体重录入 | `InputView`、`RecordController` | — |
| FR-03 BMI 计算分级 | `BmiCalculator`、`RecordController` | `body_record.bmi` |
| FR-04 体脂预测 | `BodyFatCalculator`、`RecordController` | `body_record.body_fat` |
| FR-05 历史记录管理 | `MainView`（历史区）、`RecordController`、`RecordDao`、`BodyRecord` | `body_record` |
| FR-06 动态折线图 | `ChartView`、`ChartController` | 查询 `body_record` |
| FR-07 AI 健康建议 | `AiController`、`AiService` | 外部大模型接口 |
